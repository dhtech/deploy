...
Third-generation deploy daemon: a Python 3 daemon that consumes VM
and hardware install orders from Redis and drives hypervisors/hardware.
Replaces the Python 2 `deployd` (pysphere) entirely — hard swap, no
parallel run.
...
```sh
python3 -m venv /opt/deployd
/opt/deployd/bin/pip install -r requirements.lock .
useradd -r -s /usr/sbin/nologin deployd
cp deployd.service /etc/systemd/system/
systemctl enable --now deployd
```

Configuration is Puppet-delivered in production; `config.yaml.sample` is
the template reference. Secrets come from `/etc/deployd/deployd.env`
...
`/etc/deploy.yaml` are seeded manually on the deploy server.
...
.venv/bin/pytest -m integration   # on the deploy server only: live e2e on pve-test
```
